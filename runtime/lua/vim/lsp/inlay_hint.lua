...
local function make_ranges()
  ...
  --- @param row integer
  --- @param col integer
  local function after_char(row, col)
  ...
  if mode ~= 'v' and mode ~= 'V' and mode ~= '\22' then
    ...
    return { vim.range(bufnr, row, col, row, after_char(row, col)) }
  end
  ...
  do
    ...
    if start_pos[4] > 0 then
      start_col = after_char(row, start_col)
    end
    if end_pos[4] == 0 then
      end_col = after_char(row, end_col)
    end
...
function M.action(action, opts)
  ...
  -- Validate the whole list before scheduling work, and avoid mutating cached hints.
  ...
  for _, item in ipairs(hints) do
    ...
    table.insert(hints_by_clients[item.client_id], vim.deepcopy(item.inlay_hint, true))
  end
  ...
  local function do_action(idx)
    ...
    local client_hints = hints_by_clients[client.id]
